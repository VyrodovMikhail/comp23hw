...
let convert_apply func first_arg args =
  let arg_list = first_arg :: args in
  let make_apply func arg =
    let func_typ = Lambda_lifting.get_meta func in
    let new_func_typ = Closure.delete_arrow func_typ in
    Lambda_lifting.Apply (func, arg, [], new_func_typ)
  in
  let new_apply = Base.List.fold arg_list ~init:func ~f:make_apply in
  new_apply
;;
...
let rec expr_to_cexpr =
  ...
  | Value (value, t) -> return (ast_value_to_imm_expr value, [])
  ...
  | Apply (func, first_arg, arg_list, app_typ) ->
    ...
    let new_expr = CApply (new_func_var, new_first_arg_var, new_args_vars, app_typ) in
...
let make_anf stmts =
  let rec anf_helper acc =
    ...
    | stmt :: stmts ->
      let constructor =
        match stmt with
        | Define (_, _, _, _, _) -> define
        | RecDefine (_, _, _, _, _) -> rec_define
      in
